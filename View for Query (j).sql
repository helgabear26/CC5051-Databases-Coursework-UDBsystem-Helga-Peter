...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for View V_STAFF_WORKING_OVER_6_HOURS
--------------------------------------------------------

  CREATE OR REPLACE FORCE NONEDITIONABLE VIEW "SYSTEM"."V_STAFF_WORKING_OVER_6_HOURS" ("LASTNAME", "MODULE_TITLE", "HOURSPERWEEK") AS 
  SELECT a.lastname, m.title AS module_title, atm.hoursperweek
FROM academicstaff a   
INNER JOIN academicstaffteachesmodule atm ON a.staffno = atm.academicstaff_staffno
